...
<body class="bg-slate-100" style="font-family: Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
<div class="max-w-4xl mx-auto py-12 px-4">
    <section class="rounded-3xl border border-slate-800 bg-gradient-to-r from-slate-950 via-blue-950 to-slate-900 p-8 text-white mb-8">
        ...
        <p class="mt-3 text-slate-200">Use your shipment credentials to check route status and latest location updates.</p>
    </section>
    ...
    <form method="POST" action="{{ route('tracking.public.search') }}" class="grid md:grid-cols-2 gap-3 mb-8 bg-white/95 p-6 rounded-2xl border border-slate-200 shadow-lg">
        @csrf
        <input name="tracking_code" class="rounded-xl border border-slate-200 px-3 py-2.5" placeholder="Tracking code" required>
        <input name="public_access_code" class="rounded-xl border border-slate-200 px-3 py-2.5" placeholder="Access code" required>
